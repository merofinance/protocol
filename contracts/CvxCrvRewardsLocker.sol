...
contract CvxCrvRewardsLocker is ICvxCrvRewardsLocker, Authorization {
     ...
     */
    function setSpendRatio(uint256 _spendRatio) external override onlyGovernance {
        require(
     ...
     */
    function claimRewards(bool lockAndStake) external override {
     ...
     */
    function processExpiredLocks(bool relock) external override {
        if (prepareWithdrawal) {
            ICvxLocker(CVX_LOCKER).withdrawExpiredLocksTo(_treasury());
        } else {
     ...
     */
    function withdraw(address token) external override onlyGovernance {
    ...
    function withdrawCvxCrv(uint256 amount) external override onlyGovernance {
        ...
        if (cvxcrvBal > 0) {
            IERC20(CVX_CRV).safeTransfer(_treasury(), cvxcrvBal);
        }
    }
     ...
     */
    function lockRewards() public override {
        _lockCrv();
        _lockCvx();
    }
     ...
     */
    function withdraw(address token, uint256 amount) public override onlyGovernance {
    ...
    function _unstakeCvxCrv(uint256 amount, bool withdrawal) internal {
        IRewardStaking(CVX_CRV_STAKING).withdraw(amount, true);
        if (withdrawal) {
            IERC20(CVX_CRV).safeTransfer(_treasury(), amount);
        }
    }

    function _treasury() internal view returns (address) {
        address treasury_ = _addressProvider.getTreasury();
        require(treasury_ != address(0), Error.ZERO_ADDRESS_NOT_ALLOWED);
        return treasury_;
    }
}
