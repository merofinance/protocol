...
contract VestedEscrow is IVestedEscrow, ReentrancyGuard {
    ...
    event FundAdminSet(address newFundAdmin);
    ...
    function setFundAdmin(address _fundadmin) external override {
        ...
        emit FundAdminSet(_fundadmin);
    }

    function initializeUnallocatedSupply() external override {
        require(msg.sender == admin, Error.UNAUTHORIZED_ACCESS);
    ...
    function fund(FundingAmount[] calldata amounts) external override nonReentrant {
    ...
    function claim(address _recipient) public virtual override nonReentrant {
        require(_recipient != address(0), Error.ZERO_ADDRESS_NOT_ALLOWED);
    ...
    function _computeVestedAmount(uint256 locked, uint256 _time) internal view returns (uint256) {
        ...
        return Math.min((locked * _time.uncheckedSub(startTime)) / totalTime, locked);
    }
