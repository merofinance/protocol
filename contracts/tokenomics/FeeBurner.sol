...
/**
 * The Fee Burner converts all of the callers Mero LP Tokens to a single target Mero LP Token.
...
contract FeeBurner is IFeeBurner {
    ...
    event Burned(address indexed targetLpToken, uint256 amountBurned); // Emitted after a successful burn to target lp token

    constructor(address addressProvider_) {
        _addressProvider = IAddressProvider(addressProvider_);
    }

    receive() external payable {} // Receive function for withdrawing from Mero ETH Pool

    /**
     * @notice Converts callers Tokens to target Mero LP Token for the given tokens_.
        ...
        returns (uint256 received)
    {
        ...
        for (uint256 i; i < tokens_.length; i = i.uncheckedInc()) {
            ...
            if (address(token_) == address(0)) {
                if (msg.value == 0 || burningEth_) continue;
                burningEth_ = true;
        ...
        // Transferring LP tokens back to sender
