...
@pytest.fixture
def setup_amm_gauge(
    inflation_manager, minter, ammGauge, mockAmmToken, admin, meroToken
):
    inflation_manager.setMinter(minter, {"from": admin})
    inflation_manager.setAmmGauge(mockAmmToken, ammGauge, {"from": admin})
    inflation_manager.updateAmmTokenWeight(mockAmmToken, 1e18, {"from": admin})
...
@pytest.mark.usefixtures("setup_amm_gauge")
def test_single_amm_staking_works_across_inflation_update(
    minter, ammGauge, mockAmmToken, alice, chain
):
    mockAmmToken.mint(alice, 4e18)
    mockAmmToken.approve(ammGauge, 4e18, {"from": alice})

    amm_rate = minter.getAmmInflationRate()
    decayTime = minter.lastInflationDecay() + (365*86400)
    ammGauge.stake(4e18, {"from": alice})
    start_time = ammGauge.ammLastUpdated()
    assert ammGauge.totalStaked() == 4e18

    chain.sleep(380 * 86400)
    # The checkpoint triggers and inflation rate update
    ammGauge.poolCheckpoint()
    new_amm_rate = minter.getAmmInflationRate()

    end_time = chain[-1]["timestamp"]

    expected = amm_rate * (decayTime - start_time) + new_amm_rate * (end_time - decayTime)
    assert pytest.approx(ammGauge.ammStakedIntegral(), abs=1e18) == expected / 4
