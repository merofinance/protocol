...
@pytest.fixture
def registerSetUp(address_provider, topUpAction, admin, pool, mockTopUpHandler):
    address_provider.addPool(pool, {"from": admin})
    update_topup_handler(
        topUpAction, PROTOCOL_1_ADDRESS, mockTopUpHandler, admin
    )
...
def test_withdrawal_fee_unchanged_on_action_reset(
    ...
    tx = topUpAction.register(
        ...
        {"from": th.account, "value": scale(5, 9) * topUpAction.estimatedGasUsage()},
    )
...
def test_action_resetting_is_for_free_with_unstake(
    ...
    value = scale(25, 9) * topUpAction.estimatedGasUsage()
    tx = topUpAction.register(
        encode_account(bob),
        PROTOCOL_1_ADDRESS,
        th.scale_amount(50),
...
def test_withdrawal_fee_cannot_be_avoided_by_position_registration(
    ...
    value = scale(50, 9) * topUpAction.estimatedGasUsage()
...
def test_position_registration_works_after_transfer_in_staker_vault(
    ...
    assert stakerVault.accountInfo(BobHelper.account)["balance"] == th.scale_amount(100)
