...
def registerSetUp(address_provider, topUpAction, admin, pool, mockTopUpHandler):
    ...
    update_topup_handler(topUpAction, PROTOCOL_2_ADDRESS, mockTopUpHandler, admin)
...
def test_withdraw_all_allocated_vault_and_strategy(pool, admin, coin, initialAmount):
    pool.shutdownStrategy({"from": admin})
...
def test_rebalance_if_backed_positions_exist(
    ...
    topUpAction.register(
        ...
        {"from": alice, "value": scale(10, 9) * topUpAction.estimatedGasUsage()},
    )
