...
from support.utils import get_init_code, scale
...
@pytest.fixture
def lpTokenInitCode(LpToken):
    return get_init_code(LpToken)
...
@pytest.fixture
def stakerVaultImpl(StakerVault, admin, address_provider):
    return admin.deploy(StakerVault, address_provider)
...
def setUpFactory(
    ...
    poolFactory.addLpTokenImplementation(
        LP_TOKEN_IMPL_NAME, lpTokenImpl, lpTokenInitCode
    )
    ...
    erc20PoolImpl.initialize("", coin, ZERO_ADDRESS, scale("0.03"), 0)
    ethPoolImpl.initialize("", ZERO_ADDRESS, scale("0.03"), 0)
...
def test_add_lp_token_implementation(poolFactory, lpTokenImpl, lpTokenInitCode):
    poolFactory.addLpTokenImplementation(
        LP_TOKEN_IMPL_NAME, lpTokenImpl, lpTokenInitCode
    )
    assert poolFactory.implementations(LP_TOKEN_KEY, LP_TOKEN_IMPL_NAME) == lpTokenImpl
    assert poolFactory.lpTokensInitCode(lpTokenImpl) == lpTokenInitCode


def test_add_existing_lp_token_implementation_returns_false(
    poolFactory, lpTokenImpl, lpTokenInitCode
):
    tx = poolFactory.addLpTokenImplementation(
        LP_TOKEN_IMPL_NAME, lpTokenImpl, lpTokenInitCode
    )
    assert tx.return_value
    tx = poolFactory.addLpTokenImplementation(
...
def _create_deploy_args(pool_name, underlying, pool_impl=None):
    ...
    return [
        ...
        [pool_name, scale("0.03"), 0], # pool args (name, max withdrawal fee, min withdrawal fee)
