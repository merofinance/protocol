...
def setup_pool(admin, LpToken, symbol, decimals, pool, StakerVault, controller, address_provider):
    ...
    lpToken.initialize(symbol + " - Mero LP", "mero" +
...
def pools(
    ...
    pool.initialize("ETH Pool", ZERO_ADDRESS, scale("0.03"), 0, {"from": admin})
    ...
    for token in tokens:
        ...
        symbol = token.symbol()
        pool.initialize(symbol + " Pool", token, ZERO_ADDRESS, scale("0.03"), 0, {"from": admin})
