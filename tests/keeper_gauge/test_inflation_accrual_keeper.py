...
def test_reporting_fees_correctly_updates_total_inflation(
    ...
    inflation_manager.updateKeeperPoolWeight(pool, 0.5 * 1e18, {"from": admin})
    ...
    epochTotalInflationShould = keeperInflation * \
...
def test_correct_inflation_share_per_keeper(
    ...
    assert pytest.approx(tx.events["TokensMinted"]
                         [0]["amount"]) == tx.return_value
    assert pytest.approx(tx.events["TokensMinted"]
                         [0]["amount"]) == aliceInflationShould
...
def test_correct_keeper_inflation_interrupted_fee_reporting(
    ...
    alice_inflation_should += keeperInflation * \
        (endTimeStamp - firstEpochTimeStamp)
    ...
    assert tx.events["TokensMinted"][0]["beneficiary"] == alice
    assert (
        pytest.approx(tx.events["TokensMinted"][0]
                      ["amount"]) == alice_inflation_should
    )
...
def test_claiming_works_after_kill(
    ...
    inflation_manager.callKillKeeperGauge(keeperGauge, {"from": admin})
    endTimeStamp = chain[-1]["timestamp"]
    alice_inflation_should += keeperInflation * \
        (endTimeStamp - firstEpochTimeStamp)
...
def test_inflation_correct_when_transcending_inflation_update(
    admin,
    keeperGauge,
    topUpActionFeeHandler,
    alice,
    ...
    keeper_rate = minter.getKeeperInflationRate()
    decayTime = minter.lastInflationDecay() + (365*86400)

    # Sleep until intflation update is almost there
    chain.sleep(300 * 86400)
    chain.mine()
    inflation_manager.advanceKeeperGaugeEpoch(pool, {"from": admin})
    startTimestamp = keeperGauge.lastUpdated()
    ...
    chain.sleep(80 * 86400)
    chain.mine()
    inflation_manager.advanceKeeperGaugeEpoch(pool, {"from": admin})
    end_time = chain[-1]['timestamp']
    keeper_rate_new = minter.getKeeperInflationRate()
    assert keeper_rate != keeper_rate_new

    expected = keeper_rate * (decayTime - startTimestamp) + \
        keeper_rate_new * (end_time - decayTime)

    assert keeperGauge.perPeriodTotalInflation(1) == expected
