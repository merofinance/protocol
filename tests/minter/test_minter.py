...
@pytest.fixture
def setup_minter(
    inflation_manager, admin, mockAmmGauge, mockAmmToken, mockKeeperGauge, pool, minter
):
...
def test_mint_non_inflation_tokens(minter, alice, meroToken, admin):
...
def test_mint_non_inflation_tokens_subject_to_max(minter, alice, meroToken, admin):
    with brownie.reverts("Maximum non-inflation amount exceeded."):
        tx = minter.mintNonInflationTokens(
            alice, 1000_000_000e18, {"from": admin})


def test_update_inflation_schedule(minter, alice, meroToken, admin, chain):
    ...
    assert pytest.approx(new_inflation_rate,
                         abs=1e12) == 0.6 * current_inflation_rate


def test_update_inflation_schedule_multiple_periods(
    minter, alice, meroToken, admin, chain
):
...
def test_inflation_safety_check_works(
    minter, alice, meroToken, admin, controller, chain
):
    chain.mine()
    inflation_should = (
    ...
    tx = minter.mint_for_testing_with_checks(
        alice, inflation_should, {"from": admin})
    assert tx.events["TokensMinted"][0]["beneficiary"] == alice
    assert (
...
def test_long_term_inflation_is_correct(minter, chain):
    ...
    assert pytest.approx(minter.totalAvailableToNow(),
                         abs=1e20) == expected_amount


def test_long_term_inflation_is_correct_with_delayed_updates(minter, chain):
    current_inflation_rate_lp = minter.getLpInflationRate()
    current_inflation_rate_keeper = minter.getKeeperInflationRate()
    current_inflation_rate_amm = minter.getAmmInflationRate()
    total_tokens_minted = 0
    for i in range(80):
        chain.sleep(365 * 87000)
        # Add the total that could be minted so far to the accrued amount
        total_tokens_minted += current_inflation_rate_lp * (365 * 86400)
        total_tokens_minted += current_inflation_rate_keeper * (365 * 86400)
        total_tokens_minted += current_inflation_rate_amm * (365 * 86400)
