...
@pytest.mark.parametrize(
    "case",
    [
        (TokenAddresses.SUSHI, True),
        (TokenAddresses.DAI, True),
        (TokenAddresses.CRV, True),
        (TokenAddresses.SPELL, True),
        (TokenAddresses.LDO, True),
        (TokenAddresses.CVX_CRV, False),
        (TokenAddresses.TRI_CRV, False),
    ],
)
def test_is_asset_supported(chainlink_price_oracle, case):
    asset, expected = case
    assert chainlink_price_oracle.isAssetSupported(asset) == expected
...
def test_mainnet_eth_feeds(chainlink_price_oracle):
    ...
    assert scale("0.00005") <= dai_price <= scale("0.009")
