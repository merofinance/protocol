...
def add_staker_vault(
    ...
    secondVault = admin.deploy(StakerVault, address_provider)
...
def test_add_staker_vault_with_inflation(
    ...
    gauge = admin.deploy(LpGauge, address_provider, secondVault)
    secondVault.initializeLpGauge(gauge, {"from": admin})
    assert inflation_manager.gauges(gauge)


def test_add_staker_vault_fail(admin, controller, stakerVault, lpToken, StakerVault, address_provider):
