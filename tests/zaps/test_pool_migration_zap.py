...
def oldAddressProvider(
    ...
    address_provider = admin.deploy(AddressProvider)
    role_manager = admin.deploy(RoleManager, address_provider)
    role_manager.initialize({"from": admin})
    address_provider.initialize(role_manager, treasury, {"from": admin})
    ...
    address_provider.initializeInflationManager(inflation_manager, {"from": admin})
    address_provider.initializeAddress(
    ...
    for pool_data in pool_datas:
        ...
        if underlying == TokenAddresses.ETH:
            ...
            pool.initialize(symbol + " Pool", ZERO_ADDRESS, scale("0.03"), 0, {"from": admin})
        else:
            pool = admin.deploy(Erc20Pool, controller)
            pool.initialize(symbol + " Pool", underlying, ZERO_ADDRESS, scale("0.03"), 0, {"from": admin})
        lpToken = admin.deploy(LpToken)
        lpToken.initialize(
            symbol + " - Mero LP", "mero" + symbol, decimals, pool, {"from": admin}
        )
        ...
        pool.updateMinWithdrawalFee(0, {"from": admin})
        pool.updateMaxWithdrawalFee(0, {"from": admin})
