...
def test_register_fail_with_missing_gas_cost(alice, bob, topUpAction, coin, lpToken):
    with reverts("value too low to cover gas"):
        # max gas price is 5 so we need 5 * topUpAction.estimatedGasUsage()
        topUpAction.register(
            encode_account(bob),
            PROTOCOL_1_ADDRESS,
            scale(5),
            ...
            {"from": alice, "value": scale(1, 9) * topUpAction.estimatedGasUsage()},
        )
...
def test_register(alice, bob, lpToken, topUpAction, topUpKeeperHelper, coin, gas_bank):
    ...
    eth_required_for_gas = 3 * scale(2, 9) * topUpAction.estimatedGasUsage()
