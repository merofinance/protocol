...
def test_register_fails_with_unsupported_protocol(
    ...
    eth_required_for_gas = topup_count * max_fee * topUpAction.estimatedGasUsage()
...
def test_reset_position(
    topUpAction, topUpKeeperHelper, decimals, lpToken, dai, alice, pool, stakerVault
):
    total_topup_amount = scale(10, decimals)
    single_topup_amount = scale(2, decimals)
    lpToken.approve(topUpAction, total_topup_amount, {"from": alice})
    dai.approve(pool, scale(100, decimals), {"from": alice})
    pool.depositAndStake(scale(100, decimals), 0, {"from": alice})

    max_fee = scale(50, 9)
    eth_required_for_gas = 5 * max_fee * topUpAction.estimatedGasUsage()
    topup_record = TopUpRecord(
    ...
    stakerVault.approve(topUpAction, total_topup_amount, {"from": alice})
    topUpAction.register(
        encode_account(alice),
        AAVE_PROTOCOL,
        0,
...
@pytest.mark.mainnetFork
def test_execute_aave(topUpAction, decimals, lpToken, dai, alice, pool, bob, interface, chain):
...
def test_execute_compound(
    topUpAction, decimals, lpToken, dai, alice, pool, bob, interface, chain
):
    # Depositing into pool
    total_topup_amount = scale(10, decimals)
    single_topup_amount = scale(2, decimals)
...
def test_execute_aave_repay_debt(
    topUpAction, decimals, lpToken, dai, alice, pool, bob, crv, interface, chain
):
    # Depositing into pool
    total_topup_amount = scale(10, decimals)
    single_topup_amount = scale(2, decimals)
...
def usdtPoolSetUp(
    ...
    pool.initialize("DAI Pool", usdt, ZERO_ADDRESS, scale("0.03"), 0, {"from": admin})
