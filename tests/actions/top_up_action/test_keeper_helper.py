...
def create_aave_position(
    ...
    topUpAction.register(
        ...
        {"from": user, "value": scale(25, 9) * topUpAction.estimatedGasUsage()},
    )
...
def test_returns_executable_aave_position(
    ...
    assert executable_positions[0][3][4] == dai
    assert executable_positions[0][3][5] == lpToken
    assert executable_positions[0][3][6] == SINGLE_TOPUP_AMOUNT
    assert executable_positions[0][3][7] == TOTAL_TOPUP_AMOUNT
    assert executable_positions[0][3][8] == TOTAL_TOPUP_AMOUNT
    assert executable_positions[0][3][9] == "0x" + encode_abi(["bool"], [False]).hex()
