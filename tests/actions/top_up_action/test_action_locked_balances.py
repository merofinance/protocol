...
@pytest.fixture
def registerSetUp(topUpAction, admin, pool, mockTopUpHandler, address_provider):
    address_provider.addPool(pool, {"from": admin})
    update_topup_handler(topUpAction, MOCK_PROTOCOL_NAME, mockTopUpHandler, admin)
...
def setup_staker_vault_and_minter(
    ...
    inflation_manager.updateLpPoolWeight(lpToken, 0.5 * 1e18, {"from": admin})
...
def test_action_locked_balance_updates_on_registration_and_execution(
    ...
    topUpAction.register(
        ...
        {"from": alice, "value": 5 * scale(5, 9) * topUpAction.estimatedGasUsage()},
    )

    assert stakerVault.accountInfo(
        alice)["actionLockedBalance"] == total_topup_amount
    chain.sleep(1)
    ...
    assert (
        stakerVault.accountInfo(
            alice)["actionLockedBalance"] == total_topup_amount - amount_with_fees
    )
...
def test_action_locked_balance_updates_on_registration_and_reset(
    ...
    assert stakerVault.accountInfo(alice)["actionLockedBalance"] == 0
