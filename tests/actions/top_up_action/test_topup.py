...
def _create_position(
    ...
    gas_deposit = max_gas_price * topup_count * topUpAction.estimatedGasUsage()
    record = TopUpRecord(
...
def test_topup_gas_price_boost(chain, alice, bob, topUpAction, coin, lpToken, pool):
    ...
    wei_boost = topUpAction.estimatedGasUsage() * priority_fee_boost
...
def test_topup_fails_with_unsufficient_staked(
    alice, topUpAction, coin, lpToken, pool, controller, chain, admin
):
    controller.updateKeeperRequiredStakedMERO(scale(10), {"from": admin})
    record, _ = _create_position(
        alice, "1.5", coin, alice, topUpAction, pool, lpToken, chain
    )
    with reverts("Not enough MERO tokens staked"):
        topUpAction.execute(
            alice,
            encode_account(alice),
            alice,
