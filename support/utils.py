...
def get_deployer():
    ...
    if chain_id == 1:  # mainnet
        ...
        return cast(
            LocalAccount, accounts.load("mero-mainnet-deployer", BROWNIE_ACCOUNT_PASSWORD)
        )
...
def get_init_code(contract_container) -> str:
    bytecode = contract_container.bytecode
    return "0x" + bytecode[: bytecode.find(CODE_START_PREFIX, 1)]
